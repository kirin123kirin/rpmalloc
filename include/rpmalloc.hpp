...
// static FORCEINLINE void    atomic_store_ptr(atomicptr_t* dst, void* val) { *dst = val; } //unused
#endif
...
// #include <stdatomic.h>
#include <atomic>
typedef volatile std::atomic<int32_t> atomic32_t;
typedef volatile std::atomic<int64_t> atomic64_t;
typedef volatile std::atomic<void*> atomicptr_t;

static FORCEINLINE int32_t atomic_load32(atomic32_t* src) { return std::atomic_load_explicit(src, std::memory_order_relaxed); }
static FORCEINLINE void    atomic_store32(atomic32_t* dst, int32_t val) { atomic_store_explicit(dst, val, std::memory_order_relaxed); }
static FORCEINLINE int32_t atomic_incr32(atomic32_t* val) { return atomic_fetch_add_explicit(val, 1, std::memory_order_relaxed) + 1; }
static FORCEINLINE int32_t atomic_decr32(atomic32_t* val) { return atomic_fetch_add_explicit(val, -1, std::memory_order_relaxed) - 1; }
static FORCEINLINE int32_t atomic_add32(atomic32_t* val, int32_t add) { return atomic_fetch_add_explicit(val, add, std::memory_order_relaxed) + add; }
static FORCEINLINE int     atomic_cas32_acquire(atomic32_t* dst, int32_t val, int32_t ref) { return atomic_compare_exchange_weak_explicit(dst, &ref, val, std::memory_order_acquire, std::memory_order_relaxed); }
static FORCEINLINE void    atomic_store32_release(atomic32_t* dst, int32_t val) { atomic_store_explicit(dst, val, std::memory_order_release); }
#if ENABLE_STATISTICS
static FORCEINLINE int64_t atomic_load64(atomic64_t* val) { return atomic_load_explicit(val, std::memory_order_relaxed); }
static FORCEINLINE int64_t atomic_add64(atomic64_t* val, int64_t add) { return atomic_fetch_add_explicit(val, add, std::memory_order_relaxed) + add; }
// static FORCEINLINE void    atomic_store_ptr(atomicptr_t* dst, void* val) { atomic_store_explicit(dst, val, std::memory_order_relaxed); } //unused
#endif
static FORCEINLINE void*   atomic_load_ptr(atomicptr_t* src) { return atomic_load_explicit(src, std::memory_order_relaxed); }
static FORCEINLINE void    atomic_store_ptr_release(atomicptr_t* dst, void* val) { atomic_store_explicit(dst, val, std::memory_order_release); }
static FORCEINLINE void*   atomic_exchange_ptr_acquire(atomicptr_t* dst, void* val) { return atomic_exchange_explicit(dst, val, std::memory_order_acquire); }
static FORCEINLINE int     atomic_cas_ptr(atomicptr_t* dst, void* val, void* ref) { return atomic_compare_exchange_weak_explicit(dst, &ref, val, std::memory_order_relaxed, std::memory_order_relaxed); }
...
#define pointer_offset(ptr, ofs) (void*)((char*)(ptr) + (std::ptrdiff_t)(ofs))
#define pointer_diff(first, second) (std::ptrdiff_t)((const char*)(first) - (const char*)(second))
...
_rpmalloc_reallocate(heap_t* heap, void* p, size_t size, size_t oldsize, unsigned int flags) {
	if (p) {
		...
		if (EXPECTED(span->size_class < SIZE_CLASS_COUNT)) {
			...
			if (!oldsize)
				oldsize = (size_t)((std::ptrdiff_t)span->block_size - pointer_diff(p, block));
...
static void
_memory_heap_dump_statistics(heap_t* heap, FILE* file) {
